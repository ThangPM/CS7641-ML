...
def plot_learning_curve(title, train_sizes, train_scores, test_scores, ylim=None, multiple_runs=True,
                        ...
                        x_label='Training examples (%)', y_label='Accuracy (0.0 - 1.0)'):
    """
...
def plot_model_complexity_curve(title, train_sizes, train_scores, test_scores, ylim=None, multiple_runs=True,
    ...
    if chart_type == 'bar':
        ...
        ax.bar(ind + bar_width, test_points, bar_width, yerr=test_scores_std, label="Cross-validation score")
...
def plot_model_timing(title, data_sizes, fit_scores, predict_scores, ylim=None, mean_std_axis=1):
    ...
    # plt.fill_between(data_sizes, fit_scores_mean - fit_scores_std, fit_scores_mean + fit_scores_std, alpha=0.2)
    # plt.fill_between(data_sizes, predict_scores_mean - predict_scores_std, predict_scores_mean + predict_scores_std, alpha=0.2)
    # plt.plot(data_sizes, fit_scores_mean, 'o-', linewidth=1, markersize=4, label="Train/Fit time")
    # plt.plot(data_sizes, predict_scores_mean, 'o-', linewidth=1, markersize=4, label="Test/Predict time")

    ind = data_sizes
    bar_width = 2.5
    plt.bar(ind, fit_scores_mean, bar_width, yerr=fit_scores_std, label="Train/Fit time")
    plt.bar(ind + bar_width, predict_scores_mean, bar_width, yerr=predict_scores_std, label="Test/Predict time")

    # plt.set_xticks(ind + bar_width / 2)
    # if x_tick_labels is not None:
    #     ax.set_xticklabels(x_tick_labels, rotation=45)
